...
def or3f_rules(hand, bids, bid_table, candidate_list):
    ...
    if len(candidate_list) > 1:
        ...
        if priority == 9:
            ...
            # make sure it's a second suit
            my_bids = bids.get_my_bids(hand.position)
            my_opening_bid = my_bids[0]
            for index in candidate_list:
                strain = bid_table.at[index, 'strain']
                if strain == my_opening_bid.strain:
            ...
            # bid a second suit lower ranking than original bid
            my_bids = bids.get_my_bids(hand.position)
            my_opening_bid = my_bids[0]
            # get rid of any higher ranking or equal ranking to opening bid
            for index in candidate_list:
                strain = bid_table.at[index, 'strain']
                if strain >= my_opening_bid.strain:
                    candidate_list.remove(index)
            # find the longest suit in the list
